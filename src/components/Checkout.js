    ...
    <Box display="flex" flexDirection="column">
      ...
      <Stack direction="row" my="1rem">
        <Button
          ...
          //     return;
          //   } else if (newAddress.value.length < 20) {
...
const Checkout = () => {
  ...
  const performCheckout = async (token, items, addresses) => {
    const checkoutPostReq = async (token, addressId) => {
      ...
      try {
        ...
        if (e.response) {
          ...
          enqueueSnackbar(
            "Could not check out. Check that the backend is running, reachable and returns valid JSON.",
            {
              variant: "error",
            }
          );
    ...
    checkoutPostReq(token, addresses.selected);
  };
